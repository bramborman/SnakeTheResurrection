...
namespace SnakeTheResurrection
{
    public static class Program
    {
        ...
        private static void UIPlayground()
        {
            ...
            //    }
            //};
            TextBlock splash = new TextBlock()
            {
                Text = "Snake",
                TextWrapping = TextWrapping.NoWrap,
                FontSize = 15,
                ForegroundColor = (Color)Constants.ACCENT_COLOR,
                Width = Symtext.GetSymtextWidth("Snake", 15, 15),
                Height = Symtext.GetCharHeight(15),
            ...
            Window.Compositor.BeforeRendering += () =>
            {
                int left = splash.Margin.Left + (goRight ? 1 : -1);
                int top = splash.Margin.Top + (goDown ? 1 : -1);

                if (left <= 0 || left + splash.ActualWidth >= Window.Width)
                {
                    goRight = !goRight;
                }

                if (top <= 0 || top + splash.ActualHeight >= Window.Height)
                {
                    goDown = !goDown;
                }

                splash.Margin = new Thickness(left, top, splash.Margin.Right, splash.Margin.Bottom);
            };
